...
Процедура ДвижениеПоВКМ_ОсновныеНачисления() 
	...
	Запрос.Текст = 
		...
		|			(ВЫБРАТЬ
		|				ВКМ_НачиселниеЗаработнойПлатыОсновынеНачисления.Сотрудник КАК Сотрудник
		|			ИЗ
		|				Документ.ВКМ_НачислениеЗаработнойПлаты.ОсновныеНачисления КАК ВКМ_НачиселниеЗаработнойПлатыОсновынеНачисления
		|			ГДЕ
		|				ВКМ_НачиселниеЗаработнойПлатыОсновынеНачисления.Ссылка = &Ссылка
		|				И ВКМ_НачиселниеЗаработнойПлатыОсновынеНачисления.ВидРасчета = &Оклад)) КАК
...
Процедура СформироваДвиженияВыполненныеСотрудникомРаботы(Отказ)
	...
	Запрос.Текст = 
		...
		|	ВТ_Сотрудник.СуммаНачисления,
		|	ВТ_Сотрудник.НачисленоЧасов,
