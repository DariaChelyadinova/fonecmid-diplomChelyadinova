...
Асинх Процедура ОсновныеНачисленияВидРасчетаПриИзменении(Элемент)
	...
	Если ТекущиеДанные.ВидРасчета = ПредопределенноеЗначение("ПланВидовРасчета.ВКМ_ОсновныеНачисления.Отпуск") 
		И ТекущиеДанные.ГрафикРаботы <> ПолучитьГрафикРаботОтпуска() Тогда
		
			Результат = Ждать ВопросАсинх("Установить график работ - ОтпускКалендарныеДни?", 
			РежимДиалогаВопрос.ДаНет,,,"Вопрос.");	
			
			Если Результат = КодВозвратаДиалога.Да Тогда				
				ТекущиеДанные.ГрафикРаботы = ПолучитьГрафикРаботОтпуска();			
			КонецЕсли; 
...
Процедура ЗаполнитьНаСервере()
	...
	|		ЗНАЧЕНИЕ(Справочник.ВКМ_ГрафикРабот.ПустаяСсылка)) КАК ГрафикРабот,
